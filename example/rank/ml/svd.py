...
def svd_model(average_score, embedding_size, l2_factor, user_id_size, item_id_size):
    ...
    user_bias_input = tf.keras.layers.Embedding(
        ...
        name="UserBiasLayer")(
        user_id_input)
    item_id_embedding_input = tf.keras.layers.Embedding(
        input_dim=item_id_size, output_dim=embedding_size, embeddings_regularizer=tf.keras.regularizers.l2(l2_factor),
        name="ItemIdEmbeddingLayer")(
        item_id_input)
    item_bias_input = tf.keras.layers.Embedding(
        input_dim=item_id_size, output_dim=1, embeddings_regularizer=tf.keras.regularizers.l2(l2_factor),
        name="ItemBiasLayer")(
        item_id_input)

    predict_no_bias = tf.keras.layers.Dot(axes=-1)([user_id_embedding_input, item_id_embedding_input])
    predict = tf.keras.layers.Add()([user_bias_input, item_bias_input, predict_no_bias])
...
def main(_):
    model = svd_model(average_score=3.5, embedding_size=5, l2_factor=0, user_id_size=6041, item_id_size=4000)
    data_path = os.path.abspath(__file__).replace("example/rank/ml/svd.py", "data/movieLens/ml-1m/ratings.dat")
    train_data = input_fn(data_path, batch_size=64, shuffle_buffer_size=300)
